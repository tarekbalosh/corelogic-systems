      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-20">
        ...
        >
          <SectionTitle 
            ...
            align="center"
          />
          <p className="mt-6 text-xl text-foreground/60 max-w-2xl mx-auto leading-relaxed">
            ...
            <AnimatePresence mode="wait">
              {!isSubmitted ? (
                <motion.div
                  ...
                  className="p-8 md:p-10 bg-foreground/5 border border-foreground/10 backdrop-blur-xl shadow-2xl relative group overflow-hidden"
                >
                  {/* Subtle Glow Accents */}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-sm font-bold uppercase tracking-wider text-foreground/50">Full Name</label>
                        <input 
                          required
                          type="text" 
                          ...
                          className="w-full bg-foreground/5 border border-foreground/10 px-4 py-3 focus:outline-none focus:border-primary/50 transition-colors text-foreground"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-bold uppercase tracking-wider text-foreground/50">Email Address</label>
                        <input 
                          required
                          type="email" 
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-sm font-bold uppercase tracking-wider text-foreground/50">Phone Number (Optional)</label>
                        <input 
                          type="tel" 
                          placeholder="+60..."
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-bold uppercase tracking-wider text-foreground/50">Service</label>
                        <select 
                          required
                          value={formData.service}
                          onChange={(e) => setFormData({ ...formData, service: e.target.value })}
                          className="w-full bg-foreground/5 border border-foreground/10 px-4 py-3 focus:outline-none focus:border-primary/50 transition-colors text-foreground appearance-none"
                        >
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold uppercase tracking-wider text-foreground/50">Message (Optional)</label>
                      <textarea 
                        rows={5}
                        placeholder="Tell us about your project goals..."
                        value={formData.message}
                        onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                        className="w-full bg-foreground/5 border border-foreground/10 px-4 py-3 focus:outline-none focus:border-primary/50 transition-colors text-foreground resize-none"
                      />
                    </div>
                    ...
                    <button 
                      ...
                      disabled={loading}
                      className={cn(
                        "w-full py-4 bg-primary text-navy font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-white transition-all transform active:scale-[0.98]",
                ...
                <motion.div
                  ...
                  className="p-12 bg-foreground/5 border border-foreground/10 backdrop-blur-xl shadow-2xl flex flex-col items-center text-center space-y-6"
                >
                  ...
                  <button 
                    ...
                    className="text-primary font-bold uppercase tracking-widest text-sm hover:text-foreground transition-colors flex items-center gap-2"
                  >
          ...
          <div className="space-y-12">
            ...
            >
              {contactInfo.map((info, idx) => (
                <div 
                  key={info.label}
                  className="p-6 bg-foreground/5 border border-foreground/5 hover:border-foreground/20 transition-all flex flex-col gap-4 group"
                >
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-foreground/30 mb-1">{info.label}</p>
                    {info.href ? (
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <a 
                  href="tel:+601169397149" 
                  className="flex-1 min-w-[140px] px-6 py-4 bg-primary/10 border border-primary/20 hover:bg-primary hover:text-navy transition-all flex items-center justify-center gap-3 font-bold uppercase text-[11px] tracking-widest"
                >
            ...
            <motion.div
              ...
              className="aspect-video bg-foreground/5 border border-foreground/10 relative overflow-hidden group"
            >
              ...
              <div className="absolute inset-0 hidden lg:flex items-center justify-center flex-col gap-3 p-8 text-center bg-background/60 backdrop-blur-[2px] group-hover:opacity-0 transition-opacity duration-500 pointer-events-none">
              ...
              <a 
                ...
                target="_blank"
                className="absolute bottom-4 right-4 z-20 px-6 py-3 bg-primary text-navy text-[11px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-2xl flex items-center gap-2"
              >
        ...
        >
          {reasons.map((reason, idx) => (
            <div key={reason.title} className="p-8 border border-foreground/5 bg-foreground/2 hover:bg-foreground/5 transition-colors group relative">
              <div className="absolute top-0 left-0 w-1 h-0 bg-primary group-hover:h-full transition-all duration-500" />
              <div className="text-primary font-black text-2xl mb-4 italic opacity-20">0{idx + 1}</div>
        ...
        >
          <div className="absolute top-0 right-0 w-64 h-64 bg-foreground/10 blur-[100px] rounded-full" />
          <div className="relative z-10 space-y-8">
            ...
            <button className="px-10 py-5 bg-background text-foreground font-black uppercase tracking-widest hover:bg-white hover:text-navy transition-all transform hover:-translate-y-1 shadow-2xl">
